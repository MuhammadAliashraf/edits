...
class TaskService:
    ...
    def _stale_task_timeout(self, task: Dict[str, Any]) -> Optional[int]:
        status = task.get("status")
        if status == "queued":
            return self.config.queued_task_timeout_seconds
        if status == "processing":
            return getattr(self.config, "processing_task_timeout_seconds", 1800)
        return None

    def _is_stale_task(self, task: Dict[str, Any]) -> bool:
        """Detect queued/processing tasks stalled due to worker issues."""
        timeout = self._stale_task_timeout(task)
        if timeout is None:
            return False
        ...
        return (now - updated_at).total_seconds() >= timeout
    ...
    async def process_task(
        ...
        try:
            ...
            # Clear any clips from a previous (failed/cancelled) run
            ...
            async def on_transcript_ready(transcript: str) -> None:
    ...
    async def get_task_with_clips(self, task_id: str) -> Optional[Dict[str, Any]]:
        ...
        if self._is_stale_task(task):
            ...
            timeout_seconds = self._stale_task_timeout(task)
            logger.warning(
                f"Task {task_id} stuck in {status} status for over {timeout_seconds}s; marking as error"
            )
            msg = (
                "Task timed out while waiting in queue. "
                "Ensure the worker service is running and healthy (docker-compose logs -f worker)."
                if status == "queued"
                else "Task timed out during processing. The worker may have crashed — check docker-compose logs -f worker."
            )
            await self.task_repo.update_task_status(
                self.db,
                task_id,
