...
def _get_user_id_from_headers(request: Request) -> str:
    """Get user ID. Monetization on: signed auth (same as create_task/billing_summary). Off: user_id or x-igedits-user-id."""
    config = get_config()
...
async def get_performance_metrics(request: Request, db: AsyncSession = Depends(get_db)):
    """Get aggregate processing performance metrics by mode. Admin-only."""
    config = get_config()
    user_id = _get_user_id_from_headers(request)
    # Verify user is admin (simple check via config or user role)
    # For now, require explicit METRICS_ACCESS_USER_ID or skip in self-host
    metrics_access = config.config_dict.get("METRICS_ACCESS_USER_ID")
    if metrics_access and user_id != metrics_access and config.monetization_enabled:
...
async def list_dead_letter_tasks(request: Request):
    """List tasks that exhausted retries and landed in dead-letter store. Admin-only."""
    config = get_config()
    user_id = _get_user_id_from_headers(request)
