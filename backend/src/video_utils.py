...
from assemblyai import api as _aai_api, types as _aai_types
...
# so we patch `api.create_transcript` to rename the key on the outbound payload.
def _patched_create_transcript(client: _httpx.Client, request: _aai_types.TranscriptRequest):
    payload = request.dict(exclude_none=True, by_alias=True)
    _MODEL_MAP = {"best": "universal-3-pro", "nano": "universal-2"}
    if "speech_model" in payload:
        value = payload.pop("speech_model")
        if hasattr(value, "value"):
            value = value.value
        payload["speech_models"] = [_MODEL_MAP.get(value, value)]
    response = client.post(_aai_api.ENDPOINT_TRANSCRIPT, json=payload)
    if response.status_code != _httpx.codes.OK:
        raise _aai_types.TranscriptError(
            f"failed to transcribe url {request.audio_url}: {_aai_api._get_error_message(response)}",
            response.status_code,
        )
    return _aai_types.TranscriptResponse.parse_obj(response.json())
...
class VideoProcessor:
    ...
    def get_optimal_encoding_settings(
        ...
        use_nvenc = config.video_encoder == "nvenc"
        ...
        settings = {
            "high": {
                ...
                "audio_codec": "aac",
                "audio_bitrate": "256k",
                "preset": "slow" if not use_nvenc else "p4",
                "ffmpeg_params": (
                    ["-qp", "18", "-pix_fmt", "yuv420p", "-movflags", "+faststart"]
                    if use_nvenc
                    else [
            ...
            "medium": {
                ...
                "bitrate": "4000k",
                "audio_bitrate": "192k",
                "preset": "fast" if not use_nvenc else "p2",
                "ffmpeg_params": (
                    ["-qp", "23", "-pix_fmt", "yuv420p"]
                    if use_nvenc
                    else ["-crf", "23", "-pix_fmt", "yuv420p"]
                ),
            },
        }
...
def create_optimized_clip(
    ...
    try:
        ...
        # Use unique temp audio filename per output to avoid concurrency collisions
        # Create temp file in same directory as output for atomic cleanup
        temp_audio_fd, temp_audio_path = tempfile.mkstemp(suffix='.m4a', dir=str(output_path.parent))
        os.close(temp_audio_fd)
        os.unlink(temp_audio_path)  # mkstemp creates it; we just want the path
...
def apply_transition_effect(
    ...
    try:
        ...
        # Use unique temp audio filename to avoid concurrency collisions
        temp_audio_fd, temp_audio_path = tempfile.mkstemp(suffix='.m4a', dir=str(output_path.parent))
        os.close(temp_audio_fd)
...
def create_clips_with_transitions(
    ...
    logger.info(
        "Inter-clip transitions are disabled for standalone igedits exports"
    )
