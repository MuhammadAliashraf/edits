...
import { ArrowRight, Youtube, CheckCircle, AlertCircle, Loader2, Palette, Type, Paintbrush, Film, Sparkles, Upload, Monitor, Menu, X, LogOut, List, Shield, Settings, Facebook, Instagram, Globe } from "lucide-react";
...
export default function Home() {
  ...
  const [focusMode, setFocusMode] = useState<"auto" | "center" | "left" | "right">("auto");
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" width={28} height={28} className="rounded-lg flex-shrink-0" aria-hidden="true">
                <defs>
                ...
                <rect x="0" y="0" width="100" height="100" rx="22" fill="url(#ig-hdr)"/>
                <rect x="16" y="22" width="68" height="56" rx="12" fill="none" stroke="white" strokeWidth="4.5"/>
                <circle cx="50" cy="50" r="14" fill="none" stroke="white" strokeWidth="4"/>
                <circle cx="74" cy="30" r="4" fill="white"/>
                <text x="50" y="54.5" textAnchor="middle" fill="white" fontFamily="Arial Black, Arial, sans-serif" fontSize="9.5" fontWeight="900" letterSpacing="-0.3">edits</text>
              </svg>
              <h1 className="text-xl font-bold ig-gradient-text">igedits</h1>
            </div>
          ...
          <div className="md:hidden border-t bg-background absolute left-0 right-0 z-50 shadow-lg">
            <div className="px-4 py-3 space-y-1">
              ...
              <Link
                href="/list"
                onClick={() => setMobileMenuOpen(false)}
                className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-foreground/80 hover:bg-muted/50 transition-colors"
              >
      ...
      <div className="max-w-6xl mx-auto px-6 py-10">
        ...
        {latestTask && (
          <Link href={`/tasks/${latestTask.id}`} className="block mb-8">
            <div className="flex items-center justify-between p-4 rounded-xl border border-border bg-muted/30 hover:bg-muted/50 transition-colors group">
              <div className="flex items-center gap-4 min-w-0">
                <div className="flex-shrink-0 w-10 h-10 rounded-lg bg-foreground flex items-center justify-center">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {sourceType === "youtube" ? (
                  <div className="relative">
                    ...
                    <Input
                      ...
                      disabled={isLoading}
                      className="h-14 pl-12 text-base rounded-xl border-stone-300 focus:border-stone-500 placeholder:text-muted-foreground"
                    />
                  </div>
                ) : (
                  <div
                  ...
                  >
                    ...
                    {fileName ? (
                      ...
                      <>
                        <p className="text-sm font-medium text-foreground/80">Drop a video file here or click to browse</p>
              ...
              <Card className="border-border">
                <CardContent className="px-4 pt-0 pb-2.5 space-y-2.5">
                  ...
                  {/* Crop focus (only applies to vertical output) */}
                  {outputFormat === "vertical" && (
                    <div className="p-3 border rounded-lg bg-muted/50 space-y-2.5">
                      <div className="flex items-center gap-3">
                        <Sparkles className="w-4 h-4 text-amber-500" />
                        <div>
                          <h3 className="text-sm font-medium text-foreground">Crop focus</h3>
                          <p className="text-xs text-muted-foreground">Where to frame the 9:16 crop. Auto uses AI face detection.</p>
                        </div>
                      </div>
                      <div className="grid grid-cols-4 gap-1.5">
                        {(["auto", "left", "center", "right"] as const).map((mode) => (
                          <button
                            key={mode}
                            type="button"
                            onClick={() => setFocusMode(mode)}
                            disabled={isLoading}
                            className={`px-2 py-1.5 text-xs font-medium rounded-md transition-all capitalize ${
                              focusMode === mode
              ...
              <Card className="border-border">
                <CardContent className="px-4 pt-0 pb-2.5 space-y-2.5">
                  ...
                  >
                    ...
                    <button type="button" className="text-xs text-muted-foreground hover:text-foreground/80 transition-colors">
            ...
            <div className="lg:sticky lg:top-8">
              ...
              <div className="mx-auto" style={{ maxWidth: "300px" }}>
                ...
                <div className="mt-6 space-y-3 px-2">
                  ...
                  <div className="flex items-center justify-between text-xs text-muted-foreground">
                    <span>Size</span>
                    <span className="text-foreground/80 font-medium">{fontSize}px</span>
                  </div>
                  <Separator />
                  <div className="flex items-center justify-between text-xs text-muted-foreground">
                    <span>Color</span>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-foreground/80 font-medium">{fontColor}</span>
                    </div>
                  </div>
                  <Separator />
                  <div className="flex items-center justify-between text-xs text-muted-foreground">
      ...
      <footer className="border-t bg-background mt-12">
        <div className="max-w-7xl mx-auto px-4 py-8">
          <div className="flex flex-col items-center gap-5">
            ...
            >
              <Image src="/asal-logo.png" alt="ASAL" width={26} height={26} />
              <span className="text-sm text-muted-foreground">
                Powered by <span className="font-semibold text-foreground">ASAL</span>
              </span>
            </a>

            {/* Social links */}
            <div className="flex items-center gap-2 flex-wrap justify-center">
              <a href="https://asal.life/" target="_blank" rel="noopener noreferrer"
                className="p-2 rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted/50 transition-colors" title="Website">
                <Globe className="w-4 h-4" />
              </a>
              <a href="https://www.facebook.com/profile.php?id=61573255537897" target="_blank" rel="noopener noreferrer"
                className="p-2 rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted/50 transition-colors" title="Facebook">
                <Facebook className="w-4 h-4" />
              </a>
              <a href="https://www.instagram.com/asallifeoriginal/" target="_blank" rel="noopener noreferrer"
                className="p-2 rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted/50 transition-colors" title="Instagram">
                <Instagram className="w-4 h-4" />
              </a>
              <a href="https://www.youtube.com/@AsalLlife" target="_blank" rel="noopener noreferrer"
                className="p-2 rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted/50 transition-colors" title="YouTube">
                <Youtube className="w-4 h-4" />
              </a>
              {/* TikTok */}
              <a href="https://www.tiktok.com/@asallifeoriginal" target="_blank" rel="noopener noreferrer"
                className="p-2 rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted/50 transition-colors" title="TikTok">
                <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-2.88 2.5 2.89 2.89 0 0 1-2.89-2.89 2.89 2.89 0 0 1 2.89-2.89c.28 0 .54.04.79.1V9.01a6.33 6.33 0 0 0-.79-.05 6.34 6.34 0 0 0-6.34 6.34 6.34 6.34 0 0 0 6.34 6.34 6.34 6.34 0 0 0 6.33-6.34V8.9a8.22 8.22 0 0 0 4.8 1.54V7a4.85 4.85 0 0 1-1.03-.31z" />
                </svg>
              </a>
              {/* Threads */}
              <a href="https://www.threads.com/@asallifeoriginal" target="_blank" rel="noopener noreferrer"
                className="p-2 rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted/50 transition-colors" title="Threads">
                <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M12.186 24h-.007c-3.581-.024-6.334-1.205-8.184-3.509C2.35 18.44 1.5 15.586 1.472 12.01v-.017c.03-3.579.879-6.43 2.525-8.482C5.845 1.205 8.6.024 12.18 0h.014c2.746.02 5.043.725 6.826 2.098 1.677 1.29 2.858 3.13 3.509 5.467l-2.04.569c-1.104-3.96-3.898-5.984-8.304-6.015-2.91.022-5.11.936-6.54 2.717C4.307 6.504 3.616 8.914 3.589 12c.027 3.086.718 5.496 2.057 7.164 1.43 1.783 3.631 2.698 6.54 2.717 2.623-.02 4.358-.631 5.8-2.045 1.647-1.613 1.618-3.593 1.09-4.798-.31-.71-.873-1.3-1.634-1.751-.192 1.352-.622 2.446-1.284 3.272-.886 1.102-2.14 1.704-3.73 1.79-1.202.065-2.361-.218-3.259-.801-1.063-.689-1.685-1.74-1.752-2.964-.065-1.19.408-2.285 1.33-3.082.88-.76 2.119-1.207 3.583-1.291a13.853 13.853 0 0 1 3.02.142c-.126-.742-.375-1.332-.75-1.757-.513-.586-1.308-.883-2.363-.89h-.032c-.83 0-1.87.239-2.57 1.252l-1.722-1.196c.942-1.372 2.407-2.132 4.29-2.148 1.594.012 2.94.517 3.898 1.46 1.01 1 1.554 2.503 1.575 4.36a7.95 7.95 0 0 1 .093.042c1.248.58 2.135 1.49 2.57 2.635.703 1.853.553 4.856-1.948 7.279-1.96 1.913-4.335 2.764-7.637 2.787zm.039-8.543c-.08 0-.161.002-.242.007-1.315.074-2.104.52-2.342 1.29-.158.513-.015 1.098.386 1.565.374.433.944.67 1.6.67.076 0 .152-.003.228-.008 1.17-.066 2.012-.557 2.504-1.46.323-.594.503-1.357.535-2.268a11.57 11.57 0 0 0-2.669.204z" />
                </svg>
              </a>
            </div>

            <p className="text-xs text-muted-foreground/50">
              © {new Date().getFullYear()} ASAL. Open-source under AGPL-3.0.
            </p>
          </div>
        </div>
      </footer>
