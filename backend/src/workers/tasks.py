    ...
    async with AsyncSessionLocal() as db:
        ...
        try:
            ...
            if str(e) == "Task cancelled":
                # Clean cancellation — don't retry, DB status already set to "cancelled"
                logger.info(f"Task {task_id} was cancelled by user")
                await ctx["redis"].delete(f"task_cancel:{task_id}")
...
async def rerender_clip_focus_worker(
    ...
    """
    Background worker: re-render a single clip with its currently-set focus_side.
    The route sets focus_status='rerendering' before enqueueing; this worker
    flips it to 'ready' on success or 'error' on failure so the frontend can
    unlock the UI via polling.
    """
    from ..database import AsyncSessionLocal
    from ..services.task_service import TaskService
    from ..repositories.clip_repository import ClipRepository

    set_trace_id(f"focus-{clip_id}")
    logger.info(f"Worker re-rendering focus for clip {clip_id} (task {task_id})")

    async with AsyncSessionLocal() as db:
        try:
            task_service = TaskService(db)
            return await task_service.execute_focus_rerender(task_id, clip_id)
        except Exception as e:
            logger.exception(f"Focus re-render failed for clip {clip_id}")
            try:
                async with AsyncSessionLocal() as db_err:
                    await ClipRepository.update_focus_status(
                        db_err, clip_id, "error", str(e)[:500]
                    )
            except Exception:
...
class WorkerSettings:
    ...
    functions = [
        process_video_task,
        rerender_clip_focus_worker,
    ]
    queue_name = "igedits_tasks"
